...
// MARK: - SqfliteDarwinDatabase Private Extension
...
// MARK: - SqfliteDarwinResultSet Private Extension

@interface SqfliteDarwinResultSet ()

- (int)internalStepWithError:(NSError * _Nullable __autoreleasing *)outErr;
+ (instancetype)resultSetWithStatement:(SqfliteDarwinStatement *)statement usingParentDatabase:(SqfliteDarwinDatabase*)aDB shouldAutoClose:(BOOL)shouldAutoClose;

@end

NS_ASSUME_NONNULL_END

// MARK: - SqfliteDarwinDatabase

@implementation SqfliteDarwinDatabase
...
#pragma mark SqfliteDarwinDatabase instantiation and deallocation

+ (instancetype)databaseWithPath:(NSString *)aPath {
    return SqfliteDarwinDBReturnAutoreleased([[self alloc] initWithPath:aPath]);
}

+ (instancetype)databaseWithURL:(NSURL *)url {
    return SqfliteDarwinDBReturnAutoreleased([[self alloc] initWithURL:url]);
}
...
- (void)dealloc {
    [self close];
    SqfliteDarwinDBRelease(_openResultSets);
    SqfliteDarwinDBRelease(_cachedStatements);
    SqfliteDarwinDBRelease(_dateFormat);
    SqfliteDarwinDBRelease(_databasePath);
    SqfliteDarwinDBRelease(_openFunctions);
...
+ (SInt32)SqfliteDarwinDBVersion {
    
    // we go through these hoops so that we only have to change the version number in a single spot.
    static dispatch_once_t once;
    static SInt32 SqfliteDarwinDBVersionVal = 0;
    
    dispatch_once(&once, ^{
        NSString *prodVersion = [self SqfliteDarwinDBUserVersion];
        ...
        for (NSUInteger i = 0; i < 3; i++) {
            ...
            if (component > 15) {
                NSLog(@"SqfliteDarwinDBVersion is invalid: Please use SqfliteDarwinDBUserVersion instead.");
                component = 15;
            }
            SqfliteDarwinDBVersionVal = SqfliteDarwinDBVersionVal << 4 | component;
        }
    });
    
    return SqfliteDarwinDBVersionVal;
}
...
static int SqfliteDarwinDBDatabaseBusyHandler(void *f, int count) {
    SqfliteDarwinDatabase *self = (__bridge SqfliteDarwinDatabase*)f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &SqfliteDarwinDBDatabaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
// but for folks who don't bother noticing that the interface to SqfliteDarwinDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"SqfliteDarwinDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"SqfliteDarwinDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = SqfliteDarwinDBReturnAutoreleased([_openResultSets copy]);
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (SqfliteDarwinStatement *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (SqfliteDarwinStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(SqfliteDarwinStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(SqfliteDarwinStatement*)statement forQuery:(NSString*)query {
    NSParameterAssert(query);
    if (!query) {
        NSLog(@"API misuse, -[SqfliteDarwinDatabase setCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = SqfliteDarwinDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = SqfliteDarwinDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    SqfliteDarwinDBAutorelease(_dateFormat);
    _dateFormat = SqfliteDarwinDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    SqfliteDarwinResultSet *rs = [self executeQuery:@"PRAGMA cipher_version"];
...
#else
    SqfliteDarwinResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The SqfliteDarwinDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The SqfliteDarwinDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        NSLog(@"The SqfliteDarwinDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The SqfliteDarwinDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"SqfliteDarwinDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (SqfliteDarwinResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeQuery:sql withArgumentsInArray:nil orDictionary:arguments orVAList:nil shouldBind:true];
}

- (SqfliteDarwinResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args shouldBind:(BOOL)shouldBind {
    ...
    SqfliteDarwinStatement *statement  = 0x00;
    SqfliteDarwinResultSet *rs         = 0x00;
    ...
    SqfliteDarwinDBRetain(statement); // to balance the release below
    
    if (!statement) {
        statement = [[SqfliteDarwinStatement alloc] init];
    ...
    rs = [SqfliteDarwinResultSet resultSetWithStatement:statement usingParentDatabase:self shouldAutoClose:shouldBind];
    [rs setQuery:sql];
    ...
    SqfliteDarwinDBRelease(statement);
...
- (BOOL)bindStatement:(sqlite3_stmt *)pStmt WithArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            SqfliteDarwinDBRelease(parameterName);
...
- (SqfliteDarwinResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (SqfliteDarwinResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (SqfliteDarwinResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil shouldBind:true];
}

- (SqfliteDarwinResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    SqfliteDarwinResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil shouldBind:true];
    if (!rs && error) {
...
- (SqfliteDarwinResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError * _Nullable __autoreleasing *)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    SqfliteDarwinResultSet *rs = [self executeQuery:sql withArgumentsInArray:arrayArgs orDictionary:dictionaryArgs orVAList:args shouldBind:true];
    if (!rs) {
        if (outErr) {
...
int SqfliteDarwinDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int SqfliteDarwinDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) SqfliteDarwinDBExecuteStatementsCallbackBlock)block {
    
    int rc;
    char *errmsg = nil;
    
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? SqfliteDarwinDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
static NSString *SqfliteDarwinDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError * _Nullable __autoreleasing *)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", SqfliteDarwinDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString*)name error:(NSError * _Nullable __autoreleasing *)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", SqfliteDarwinDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString*)name error:(NSError * _Nullable __autoreleasing *)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", SqfliteDarwinDBEscapeSavePointName(name)];
...
- (BOOL)checkpoint:(SqfliteDarwinDBCheckpointMode)checkpointMode error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:nil logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(SqfliteDarwinDBCheckpointMode)checkpointMode name:(NSString *)name error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:name logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(SqfliteDarwinDBCheckpointMode)checkpointMode name:(NSString *)name logFrameCount:(int *)logFrameCount checkpointCount:(int *)checkpointCount error:(NSError * __autoreleasing *)error
{
...
void SqfliteDarwinDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void SqfliteDarwinDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString *)name arguments:(int)arguments block:(void (^)(void *context, int argc, void **argv))block {
    ...
    id b = SqfliteDarwinDBReturnAutoreleased([block copy]);
    
    [_openFunctions addObject:b];
    
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (void*)b, &SqfliteDarwinDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (__bridge void*)b, &SqfliteDarwinDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
...
// MARK: - SqfliteDarwinStatement

@implementation SqfliteDarwinStatement
